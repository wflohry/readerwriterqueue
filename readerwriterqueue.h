...
class MOODYCAMEL_MAYBE_ALIGN_TO_CACHELINE ReaderWriterQueue
{
  ...
  private:
...
#endif
	{
        ...
        {
#if MOODYCAMEL_HAS_EMPLACE
            return inner_enqueue_<canAlloc>(tailBlock_, blockFront, blockTail, std::forward<Args>(args)...);
#else
            return inner_enqueue_<canAlloc>(tailBlock_, blockFront, blockTail, std::forward<U>(element));
#endif
		}
...
#if MOODYCAMEL_HAS_EMPLACE
    template <AllocationMode canAlloc, typename... Args>
    bool inner_enqueue_(void *_tailBlock_, size_t blockFront, size_t blockTail, Args &&...args) AE_NO_TSAN
#else
    template <AllocationMode canAlloc, typename U>
    bool inner_enqueue_(void *_tailBlock_, size_t blockFront, size_t blockTail, U &&element) AE_NO_TSAN
#endif
    {
        Block *tailBlock_ = (Block *) _tailBlock_;
        fence(memory_order_acquire);
